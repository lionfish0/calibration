    ...
    def matrix(self,X1,X2):    
        ...
        for index_param,indices_param in enumerate(self.indices): #loop over the parameters
            for index_sensor,index in enumerate(indices_param): #loop over the sensors
                ...
                cov[np.ix_(keep1,keep2)] = self.gpflowkernels[index].K(X1[keep1,:],X2[keep2,:])
...
#>>>>>>> 6d36bfb75f81650ea51ec439eaa9b869b676acff
    ...
    def computeforminibatch(self,justuserefs=False):    
        if justuserefs: #to start with we just train on data from pairings with reference sensors to help optimisation
            ...
            keep = np.where((np.isin(self.fullX[:,1],np.where(self.refsensor)[0])) | (np.isin(self.fullX[:,2],np.where(self.refsensor)[0])))[0]
        ...
        self.X = np.c_[np.tile(np.r_[np.c_[self.fullX[batch,0],self.fullX[batch,1]],np.c_[self.fullX[batch,0],self.fullX[batch,2]]],[self.C,1]),np.repeat(np.arange(self.C),2*self.minibatchsize)] 
...
##############Previous code without minibatches #################
class SparseModelNoMiniBatch():
...
class CalibrationSystemNoMiniBatch():
    ...
    def precompute(self):    
        ...
        M = self.Z.shape[0]
        self.mu = tf.Variable(0.3*tf.random.normal([M,1]))
        self.scale = tf.Variable(0.05*np.tril(1.0*np.random.randn(M,M)+1.0*np.eye(M)),dtype=tf.float32)        
        ...
        if self.likemodel=='distribution' or self.likemodel=='process':
            ...
            self.pulike = tfd.MultivariateNormalFullCovariance(mu_u,cov_u+np.eye(cov_u.shape[0])*self.jitter)
             #0.8
            self.smlike = SparseModelNoMiniBatch(self.Xlike,self.Zlike,1,self.k)
        else:
            self.mulike = None
        if self.likemodel=='process':
            self.scalelike = tf.Variable(1e-10*np.eye(Mlike),dtype=tf.float32)
